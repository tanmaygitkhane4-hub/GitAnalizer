...
import { syncGitHub, startAnalysis, isAuthenticated } from "@/lib/api";
...
export default function Input() {
  ...
  const launch = async () => {
    ...
    try {
      if (!isAuthenticated()) {
        // Store intent and send to GitHub OAuth
        localStorage.setItem('pendingGithub', github);
        localStorage.setItem('pendingLevel', level ?? '');
        localStorage.setItem('pendingLocation', remote ? 'remote' : location);
        window.location.href = `${import.meta.env.VITE_API_URL || 'http://localhost:4000'}/api/auth/github`;
        return;
      }
      // User is authenticated — start sync + analysis
      await syncGitHub();
      await startAnalysis();
